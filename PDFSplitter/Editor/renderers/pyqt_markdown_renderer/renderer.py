# editor/renderers/pyqt_markdown_renderer/renderer.py
...
from ...text_document_model import TextDocumentModel

class Renderer(BaseRenderer):
    """
    PyQt WebEngine-based Markdown renderer that uses WebEngine for
    ...
    def open_dual_tabs(self, file_path: str) -> tuple:
        ...
        try:
            ...
            self.logger.error("PyQtMarkdownRenderer", f"Failed to read file {file_path}: {e}")
            return None, None
        
        return self.load_markdown_dual_tabs(file_path, content)
        
    def load_markdown_dual_tabs(self, file_path: str, content: str) -> tuple:
        ...
        if not PYQT_AVAILABLE:
            messagebox.showwarning(
                ...
                "PyQt6 WebEngine is not available. Install with: pip install PyQt6 PyQt6-WebEngine.\n\nFalling back to standard markdown renderer."
            )
            ...
            try:
                ...
                self.logger.error("PyQtMarkdownRenderer", "Failed to fallback to standard markdown renderer")
                return None, None
    ...
    def _create_editor_tab(self, file_path: str, content: str) -> str:
        ...
        try:
            ...
            # Create a document model for the Markdown file
            ...
            self.logger.error("PyQtMarkdownRenderer", f"Failed to create editor tab: {e}")
            return None

    def _create_preview_tab(self, file_path: str, content: str) -> str:
        """Create a PyQt WebEngine-based preview tab for the markdown file."""
        try:
            # Create a tab frame
            tab_frame = ttk.Frame(self.editor.tab_manager.notebook)
            ...
            browser_frame = PyQtWebFrame(tab_frame, html_content=html_content)
            ...
            self.logger.error("PyQtMarkdownRenderer", f"Failed to create preview tab: {e}")
            return None
...
"""
            ...
            self.logger.error("PyQtMarkdownRenderer", f"Markdown conversion failed: {e}")
    ...
    def _update_markdown_preview(self, preview_tab_id: str, markdown_content: str):
        ...
        try:
            ...
            self.logger.error("PyQtMarkdownRenderer", f"Failed to update preview: {e}")
    ...
    def refresh_preview(self, editor_tab_id: str):
        ...
        try:
            ...
            self.logger.error("PyQtMarkdownRenderer", f"Failed to refresh preview: {e}")
    ...
    def open(self, path: str) -> str:
        ...
        try:
            ...
            self.logger.error("PyQtMarkdownRenderer", f"Failed to open file: {e}")
            return None
    ...
    def scroll(self, tab_id: str, direction: str, amount: int):
        ...
        try:
            ...
            self.logger.error("PyQtMarkdownRenderer", f"Scroll failed: {e}")

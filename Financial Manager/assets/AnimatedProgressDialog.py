...
class AnimatedProgressDialog(QDialog):
    ...
    def get_instance(cls, parent=None, title="Working...", message="Please wait..."):
        ...
        # Check if we have an active instance that is still valid and visible
        if cls._active_instance is not None:
            try:
                # Try to check if it's still valid (not deleted) and visible
                if cls._active_instance.isVisible() and cls._active_instance.isEnabled():
                ...
                # Instance was deleted or is invalid, reset it
    ...
    def __del__(self):
        ...
        try:
            if hasattr(self, "_spinner_thread") and self._spinner_thread is not None:
                if self._spinner_thread.isRunning():
                    self._spinner_thread.stop()
                    self._spinner_thread.wait(1000)
        except:
            pass
    ...
    def closeEvent(self, event):
        """Handle dialog close - ensure all threads are properly stopped"""
        try:
            # 1) Stop the spinner thread first
            if hasattr(self, "_spinner_thread") and self._spinner_thread is not None:
                try:
                    # Disconnect signals to prevent new updates
                    self._spinner_thread.tick.disconnect()
                except:
                    pass
                
                self._spinner_thread.stop()
                # Wait for thread to finish with timeout
                if not self._spinner_thread.wait(2000):  # 2 second timeout
                    # If thread doesn't stop gracefully, terminate it
                    self._spinner_thread.terminate()
                    self._spinner_thread.wait()  # Wait for termination to complete
                
                self._spinner_thread = None

            # 2) Then stop any worker thread
            if getattr(self, "worker_thread", None) and self.worker_thread is not None:

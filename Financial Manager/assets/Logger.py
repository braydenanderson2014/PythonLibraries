#!/usr/bin/env python3
"""
Enhanced Logger with proper file size rotation and Qt signal support
"""

import logging
from logging.handlers import RotatingFileHandler
from io import StringIO, TextIOWrapper
import os
import sys
import atexit
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'src')))

from src.pyinstaller_helpers import get_config_dir

# Try to import PyQt6 for signal support
try:
    from PyQt6.QtCore import QObject, pyqtSignal
    PYQT_AVAILABLE = True
except ImportError:
    PYQT_AVAILABLE = False
    # Create dummy QObject and pyqtSignal for when PyQt is not available
    class QObject:
        pass
    def pyqtSignal(*args, **kwargs):
        return None


class InMemoryLogHandler(logging.Handler):
    def __init__(self):
        super().__init__()
        self.log_stream = StringIO()

    def emit(self, record):
        if self.log_stream.closed:
            # Create a new StringIO if the current one is closed
            self.log_stream = StringIO()
            
        self.log_entry = self.format(record)
        self.log_stream.write(self.log_entry + '\n')

    def get_logs(self):
        if self.log_stream.closed:
            return "Log stream was closed"
        return self.log_stream.getvalue()

    def clear_logs(self):
        if not self.log_stream.closed:
            self.log_stream.close()
        self.log_stream = StringIO()
        
    def close(self):
        if not self.log_stream.closed:
            self.log_stream.close()
        super().close()

class EncodedStreamHandler(logging.StreamHandler):
    def __init__(self, stream=None, encoding=None):
        self.encoding = encoding
        
        if encoding and stream and hasattr(stream, 'buffer'):
            stream = TextIOWrapper(stream.buffer, encoding=encoding)
            
        super().__init__(stream)

    def emit(self, record):
        try:
            if self.stream is None:
                return
            if getattr(self.stream, 'closed', False):
                return
            
            msg = self.format(record)
            self.stream.write(msg + self.terminator)
            self.flush()
        except Exception:
            self.handleError(record)


class _LogSignalEmitter(QObject):
    """Qt signals for log events - used by splash screen and other listeners"""
    log_message = pyqtSignal(str, str, str)  # title, level, message
    
    def emit_log(self, title, level, message):
        """Emit log signal"""
        if PYQT_AVAILABLE:
            self.log_message.emit(title, level, message)

class Logger:
    _instance = None

    def __new__(cls, *args, **kwargs):
        if not cls._instance:
            cls._instance = super(Logger, cls).__new__(cls)
        return cls._instance
        
    def __init__(self, log_dir=None):
        if getattr(self, '_initialized', False):
            return  # Prevent re-initialization
        self._initialized = True
        
        # Initialize Qt signal support if available
        if PYQT_AVAILABLE:
            self._signal_emitter = _LogSignalEmitter()
        else:
            self._signal_emitter = None
        
        # Get log settings from settings controller
        self.log_file_path, self.max_size_mb, self.backup_count = self._get_log_settings(log_dir)
        
        # Create logger with name
        self.logger = logging.getLogger('financial_tracker')
        self.logger.setLevel(logging.DEBUG)
        
        # Clear any existing handlers to avoid duplicates
        if self.logger.handlers:
            self.logger.handlers.clear()
        
        # Create in-memory handler
        self.in_memory_handler = InMemoryLogHandler()
        self.in_memory_handler.setFormatter(logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s'))
        self.logger.addHandler(self.in_memory_handler)
        
        # Create rotating file handler using settings
        try:
            # Ensure log directory exists
            log_dir_path = os.path.dirname(self.log_file_path)
            if log_dir_path:
                os.makedirs(log_dir_path, exist_ok=True)
            
            # Convert MB to bytes for RotatingFileHandler
            max_bytes = int(self.max_size_mb * 1024 * 1024)
            
            self.file_handler = RotatingFileHandler(
                self.log_file_path, 
                mode='a', 
                maxBytes=max_bytes, 
                backupCount=self.backup_count,
                encoding='utf-8'
            )
            self.file_handler.setFormatter(logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s'))
            self.logger.addHandler(self.file_handler)
        except Exception as e:
            sys.stderr.write(f"Failed to create rotating file handler: {e}\n")
        
        # Create stdout handler
        try:
            self.stream_handler = EncodedStreamHandler(sys.stdout, encoding='utf-8')
            self.stream_handler.setFormatter(logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s'))
            self.logger.addHandler(self.stream_handler)
        except Exception as e:
            try:
                self.warning("Logger", f"Failed to create stream handler: {e}")
            except:
                sys.stderr.write(f"Failed to create stream handler: {e}\n")
        
        # Register clean-up to happen at exit
        atexit.register(self.cleanup)
        
        self.info("LOGGER", "=================================================================")
        self.info("LOGGER", f" EVENT: INIT - Log file: {self.log_file_path}")
        self.info("LOGGER", f" LOG ROTATION: Max size {self.max_size_mb}MB, {self.backup_count} backups")
        self.info("LOGGER", "=================================================================")
        
    def _get_log_settings(self, log_dir=None):
        """Get the log settings from settings controller or use fallbacks"""
        try:
            # Try to import and use settings controller
            from src.settings import SettingsController
            settings = SettingsController()
            
            log_file_path = settings.get('log_file_path')  # Use get() method instead
            max_size_mb = settings.get('max_log_size_mb')  # Use get() method instead
            backup_count = settings.get('log_backup_count')  # Use get() method instead
            
            # Validate log_file_path is not empty
            if not log_file_path or log_file_path.strip() == '':
                raise ValueError("log_file_path is empty")
            
            return log_file_path, max_size_mb, backup_count
        except Exception as e:
            # Fallback settings - use stderr since logger isn't initialized yet
            sys.stderr.write(f"[Logger] Warning: Could not load settings ({e}), using fallback\n")
            
            # Use PyInstaller helper to get config directory
            try:
                config_dir = get_config_dir()
                log_file_path = str(config_dir / "financial_tracker.log")
            except Exception:
                # Last resort fallback
                if log_dir:
                    log_file_path = os.path.join(log_dir, "financial_tracker.log")
                else:
                    # Use current working directory
                    log_file_path = os.path.join(os.getcwd(), "financial_tracker.log")
            
            return log_file_path, 10, 3  # Default: 10MB, 3 backups

    def log(self, title, message):
        if hasattr(self, 'logger') and self.logger:
            self.logger.info(f"[{title}]: {message}")
            self._emit_signal(title, "INFO", message)
        elif sys.stderr:
            sys.stderr.write(f"[INFO] [{title}]: {message}\n")

    def error(self, title, message):
        if hasattr(self, 'logger') and self.logger:
            self.logger.error(f"[{title}]: {message}")
            self._emit_signal(title, "ERROR", message)
        elif sys.stderr:
            sys.stderr.write(f"[ERROR] [{title}]: {message}\n")

    def warning(self, title, message):
        if hasattr(self, 'logger') and self.logger:
            self.logger.warning(f"[{title}]: {message}")
            self._emit_signal(title, "WARNING", message)
        elif sys.stderr:
            sys.stderr.write(f"[WARNING] [{title}]: {message}\n")

    def debug(self, title, message):
        if hasattr(self, 'logger') and self.logger:
            self.logger.debug(f"[{title}]: {message}")
            self._emit_signal(title, "DEBUG", message)
        elif sys.stderr:
            sys.stderr.write(f"[DEBUG] [{title}]: {message}\n")

    def critical(self, title, message):
        if hasattr(self, 'logger') and self.logger:
            self.logger.critical(f"[{title}]: {message}")
            self._emit_signal(title, "CRITICAL", message)
        elif sys.stderr:
            sys.stderr.write(f"[CRITICAL] [{title}]: {message}\n")

    def info(self, title, message):
        if hasattr(self, 'logger') and self.logger:
            self.logger.info(f"[{title}]: {message}")
            self._emit_signal(title, "INFO", message)
        elif sys.stderr:
            sys.stderr.write(f"[INFO] [{title}]: {message}\n")

    def exception(self, title, message):
        if hasattr(self, 'logger') and self.logger:
            self.logger.exception(f"[{title}]: {message}")
            self._emit_signal(title, "EXCEPTION", message)
        elif sys.stderr:
            sys.stderr.write(f"[EXCEPTION] [{title}]: {message}\n")

    def fatal(self, title, message):
        if hasattr(self, 'logger') and self.logger:
            self.logger.fatal(f"[{title}]: {message}")
            self._emit_signal(title, "FATAL", message)
        elif sys.stderr:
            sys.stderr.write(f"[FATAL] [{title}]: {message}\n")

    def get_logs(self):
        return self.in_memory_handler.get_logs()
    
    def delete_logs(self):
        """Clear in-memory logs and truncate log file"""
        self.in_memory_handler.clear_logs()

        # Close and remove all file handlers
        for handler in self.logger.handlers[:]:
            if isinstance(handler, (logging.FileHandler, RotatingFileHandler)):
                handler.close()
                self.logger.removeHandler(handler)

        # Truncate the log file instead of deleting it
        if os.path.exists(self.log_file_path):
            try:
                with open(self.log_file_path, "w", encoding="utf-8") as file:
                    file.truncate(0)  # Clear the content of the file
                    print("Log file truncated successfully.")
            except (PermissionError, OSError) as e:
                sys.stderr.write(f"Failed to truncate log file: {e}\n")
                return  # Exit if the file cannot be truncated

        # Recreate the rotating file handler
        try:
            max_bytes = int(self.max_size_mb * 1024 * 1024)
            self.file_handler = RotatingFileHandler(
                self.log_file_path, 
                mode='a', 
                maxBytes=max_bytes, 
                backupCount=self.backup_count,
                encoding='utf-8'
            )
            self.file_handler.setFormatter(logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s'))
            self.logger.addHandler(self.file_handler)
        except Exception as e:
            sys.stderr.write(f"Failed to create new rotating file handler: {e}\n")

    def get_current_log_size_mb(self):
        """Get current log file size in MB"""
        try:
            if os.path.exists(self.log_file_path):
                size_bytes = os.path.getsize(self.log_file_path)
                return size_bytes / (1024 * 1024)
        except Exception:
            pass
        return 0.0

    def manually_rotate_if_needed(self):
        """Manually check and rotate log if needed (backup method)"""
        try:
            current_size = self.get_current_log_size_mb()
            if current_size >= self.max_size_mb:
                # Force rotation by triggering the rotating handler
                if hasattr(self.file_handler, 'doRollover'):
                    self.file_handler.doRollover()
                    self.info("LOGGER", f"Log rotated - was {current_size:.2f}MB, limit {self.max_size_mb}MB")
        except Exception as e:
            self.error("LOGGER", f"Failed to rotate log: {e}")

    def cleanup(self):
        """Clean up resources when the program exits"""
        for handler in self.logger.handlers:
            try:
                handler.close()
            except:
                pass

    def _emit_signal(self, title, level, message):
        """Internal method to emit log signals"""
        if self._signal_emitter and PYQT_AVAILABLE:
            try:
                self._signal_emitter.emit_log(title, level, message)
            except Exception:
                pass  # Silently fail if signal emission fails

    def connect_log_listener(self, callback):
        """Connect a listener callback to log events
        
        Args:
            callback: Function that receives (title, level, message)
        """
        if self._signal_emitter and PYQT_AVAILABLE:
            self._signal_emitter.log_message.connect(
                lambda title, level, msg: callback(title, level, msg)
            )
        else:
            # Warn if PyQt is not available
            sys.stderr.write("[Logger] Warning: Qt not available for signal listeners\n")

# For backward compatibility

...
gearButton.style.display = 'none'; // Hidden by default, shown only on TCLP pages
...
updateTimerVisibility(); // Refresh visibility
});
...
const urlKey = isTCLP ? currentURL.split(/[?&]/)[0] : null;
...
if (isTCLP) {
gearButton.style.display = 'block'; // Always show gear on TCLP pages

// Always start save interval on TCLP pages if currentUrlKey exists
if (currentUrlKey) {
startSaveInterval();
}
...
timerDisplay.style.display = 'none';
gearButton.style.display = 'none'; // Hide gear off TCLP pages
if (timerInterval) clearInterval(timerInterval);
if (saveInterval) clearInterval(saveInterval);
stopBlinking();
}
...
// Save on visibility change (e.g., tab switch, minimize)
document.addEventListener('visibilitychange', () => {
if (document.visibilityState === 'hidden' && currentUrlKey) {
updateTimerInCache(currentUrlKey, seconds, cache);
saveCache(cache);
}
});

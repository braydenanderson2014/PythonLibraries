...
"""
    ...
    def get_additional_files(self):
        ...
        auto_detect = input("\nAuto-detect help/documentation folders? (Y/n): ").lower()
        if auto_detect not in ['n', 'no']:
            self.auto_detect_help_folders()
        
        print("\nAdditional files/folders to include (optional):")
        print("Examples: README.md, .env, config/, data/")
        print("Enter one per line, press Enter on empty line to finish:")
        
        files = self.additional_files.copy()  # Start with auto-detected files
        ...
        while True:
            ...
            if os.path.exists(file_path):
                if file_path not in files:
                    ...
                    print(f"  ⚠️  Already included: {file_path}")
            else:
    ...
    def auto_detect_help_folders(self):
        """Automatically detect and include help/documentation folders"""
        help_folders = [
            "Help Documents",
            "help",
            "docs", 
        ...
        for folder_name in help_folders:
            if os.path.exists(folder_name) and os.path.isdir(folder_name):
                # Check if folder contains help files
                try:
                    folder_contents = os.listdir(folder_name)
                    has_help_files = any(
                        f.startswith('HELP_') or f.lower().endswith(('.md', '.txt', '.rst', '.html'))
                        for f in folder_contents
                    )
                    
                    if has_help_files:
                        detected_folders.append(folder_name)
                        print(f"📚 Auto-detected help folder: {folder_name}")
                except OSError:
                    pass  # Skip folders we can't read
        
        if detected_folders:
            print(f"✅ Found {len(detected_folders)} help folder(s) to include")
            for folder in detected_folders:
                if folder not in self.additional_files:
                    self.additional_files.append(folder)
        else:
            print("ℹ️  No help folders detected")
    ...
    def quick_build_flow(self):
        ...
        if not using_previous:
            ...
            # Auto-detect help/documentation folders
            self.auto_detect_help_folders()

...
# Import the animated progress dialog
...
class PDFImportWorker(QThread):
    """Worker thread for importing large amounts of PDFs"""
    progress_updated = pyqtSignal(int, int, str)  # current, total, filename
    import_complete = pyqtSignal(int)  # number of files imported
    error_occurred = pyqtSignal(str)
    
    def __init__(self, pdf_files, existing_history):
        super().__init__()
    ...
    def run(self):
        """Import PDF files in background thread"""
        try:
            added_count = 0
            ...
            for i, pdf_file in enumerate(self.pdf_files):
                ...
                # Add to imported files if not already in history
                if pdf_file not in self.existing_history:
                    self.imported_files.append(pdf_file)
                    added_count += 1
                
                # Small delay only for UI responsiveness (reduced from 50ms)
                if i % 10 == 0:  # Only delay every 10 files
                    QThread.msleep(5)  # Much shorter delay
            
            self.import_complete.emit(added_count)
            
        except Exception as e:
            self.error_occurred.emit(f"Error during import: {str(e)}")

class PDFViewerWidget(QWidget):
    """PDF Viewer Widget for the main application"""
    
    def __init__(self, parent=None, file_list_controller=None, status_bar=None):
        ...
        self.status_bar = status_bar  # Reference to main window status bar
        self.progress_dialog = None  # For animated progress during large imports
    ...
    def load_pdf_file(self, file_path):
        ...
        try:
            ...
            self.update_status(f"Loading PDF: {filename}")
            ...
            self.update_status(f"Loaded PDF: {filename} ({self.total_pages} pages)")
    ...
    def close_pdf_file(self):
        ...
        # Update status
        self.update_status("PDF file closed")
        # Update UI
    ...
    def add_multiple_to_history(self, file_paths):
        """Add multiple PDF files to history efficiently (batch operation)"""
        if not file_paths:
            return
        
        # Process all files without updating UI each time
    ...
    def update_status(self, message):
        ...
        if self.status_bar:
            ...
            print(f"Status: {message}")  # Fallback to console if no status bar
    ...
    def import_pdfs_from_tabs(self):
        """Import PDF files from other tabs using the file list controller with progress dialog"""
        ...
        # Trust the file controller - it should already have valid files
        pdf_files = [f for f in shared_files if f.lower().endswith('.pdf')]
        ...
        new_files = [f for f in pdf_files if f not in self.pdf_history]
        if not new_files:
            QMessageBox.information(
                self,
                "No New PDFs",
                f"All {len(pdf_files)} PDF file(s) are already in the history."
            )
            return
        ...
        # Update status
        self.update_status(f"Importing {total_files} PDF files...")
        
        # Show progress dialog for large imports (more than 5 files)
        if total_files > 5:
            self.progress_dialog = AnimatedProgressDialog.get_instance(
                self,
                "Importing PDFs",
                f"Importing {total_files} PDF files from other tabs..."
            )
            self.progress_dialog.show()
            
            # Create and start worker thread
            self.import_worker = PDFImportWorker(new_files, self.pdf_history)
            ...
            # For small imports, do it synchronously without progress dialog
            files_to_add = []
            for pdf_file in new_files:
                if pdf_file not in self.pdf_history:
                    files_to_add.append(pdf_file)
            
            # Batch add to history
            self.add_multiple_to_history(files_to_add)
            added_count = len(files_to_add)
            
            # Update status and show completion message
            self.update_status(f"Imported {added_count} PDF files")
            QMessageBox.information(
                self,
                "PDFs Imported",
    ...
    def on_import_progress(self, current, total, filename):
        """Handle progress updates from import worker"""
        if self.progress_dialog:
            progress_text = f"Importing file {current} of {total}:\n{filename}"
            self.progress_dialog.update_message(progress_text)
        
        # Also update status bar
        self.update_status(f"Importing {current}/{total}: {os.path.basename(filename)}")
    
    def on_import_complete(self, added_count):
        """Handle completion of import process"""
        if self.import_worker:
            # Add all imported files to history using batch operation
            self.add_multiple_to_history(self.import_worker.imported_files)
        ...
        self.update_status(f"Successfully imported {added_count} PDF files")
        
        QMessageBox.information(
            self,
            "Import Complete",
            f"Successfully imported {added_count} PDF file(s) to the history.\n"
            "Select from the dropdown to view a PDF."
        )
